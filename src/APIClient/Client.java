...
public class Client {
   ...
   public Client( String hostName, int portNumber ) throws IOException,UnknownHostException{
    ...
    public boolean makeLoginUtente( Persona p ) {

        Messaggio m = new Messaggio("LOGINUTENTE",p) ;
        System.out.println("LOGINUTENTE "+p.getEmail()+" "+p.getPassword()) ;

        try {
            obj_out.writeObject(m) ;
            Persona ret = (Persona) obj_in.readObject();
    ...
    public Azienda makeLoginAzienda( Azienda a ) {
        ...
        try {
            obj_out.writeObject(m);
            Azienda ret = (Azienda) obj_in.readObject();
            if ( ret != null ) {
                return ret ;
            }
